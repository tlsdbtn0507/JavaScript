...
//따라서 객체는 원시 값과 다른 방식으로 동작하도록 설계되었다.

//** 자바스크립트 객체의 관리 방식
// js객체는 프로퍼티 키를 인덱스로 사용하는 해시테이블(연관배열이라고도 함)이다 대부분의 js엔진은 해시테이블과 유사하지만 높은 성능을 위해
//해시 테이블보다 나은 방법으로 쓴다
// 자바나 c++같은 클래스 기반 객체지향은 사전에 정의된 클래스를 기반으로 객체를 생성함
//즉 객체 생성 이전에 미리 프로퍼티와 메서드가 정해져있고 그대로 객체를 생성하고 생성 이후 프로퍼티를 삭제하거나 추가 할 수 없음

//하지만 js는 클래스 없이 객체를 생성할 수 있고 객체가 생성된 이후라도 동적으로 프로퍼티와 메서드를 추가 할 수 있다
//이는 사용하기 매우 편리하지만 성능, 이론적인 측면에서 클래스 기반 보다 비 효율적이다

//이 단점을 극복하기 위해 v8 js엔진부터 동적 탐색 대신 히든클래스를 사용해 c++ 객체의 프로퍼티에 접근 성능을 보장함

//7-2-1 변경 가능한 값
//객체 타입의 값, 객체는 변경 가능한 값임
//원시 값은 변경 불가능한 값이므로 원시값을 갖는 변수의 값을 변경하려면 재할당 외에는 방법이 없으나
//객체는 변경 가능한 값으로 객체를 할당한 변수는 재할당 없이 객체를 직접 변경할 수 있다 재할당 없이 프로퍼티를 동적으로 추가하거나 값을 갱신할 수도 있으며 프로퍼티 자체를
//삭제 할 수도 있다
...
//이러한 객체의 구조적 단점으로는 여러개의 식별자가 하나의 객체를 공유 할 수 있다
...
//copy와 person은 저장된 메모리 주소는 다르지만 동일한 참조 값을 갖으며 둘다 동일한 객체를 가리키고 이것이
//두개의 식별자가 하나의 객체를 공유한다(위에서 언급한 객체의 구조적 단점)는 것
...
//동일한 객체를 가리키며 어느 한쪽에 객체를 변경하면 서로 영향을 주고 받는다
console.log(copy); //=> {name:'kim',address:'Seoul'}
console.log(person); //=> {name:'kim',address:'Seoul'}

//값에 의한 전달과 참조에 의한 전달은 식별자가 기억하는 메모리 공간에 저장되어 있는 값을 복사, 전달한다는 면에서 동일
//js에는 참조에 의한 전달은 존재하지 않고 값에 의한 전달만이 존재한다
...
console.log(person1 === person2); //=> false p1과 p2의 변수가 가리키는 객체는 내용만 같지 다른 메모리에 저장된 별개의 객체
